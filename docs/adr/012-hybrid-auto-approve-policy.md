...
#### ④ 自動承認の重複実行防止と【人間のレビュー決定および議論の絶対尊重（ガバナンス）】
*   自動承認を実行する前に、PRに投稿されている既存のレビューおよびスレッドをスキャンします。すでに本アクション（またはBotアカウント）による `APPROVED` のレビューが存在する場合、多重にレビューを作成してPRタイムラインをスパム化するのを防ぐため、自動承認プロセスを安全にスキップ（冪等性を担保）します。
*   **【人間のレビュー・対話の絶対尊重（未解決スレッドの考慮）】**: 
    *   人間の先輩レビュアーの意思決定とのコンフリクトを回避するため、既存レビューの走査時に `CHANGES_REQUESTED`（変更要求/却下）のレビューが1つでも存在する場合は、AIによる自動承認（Approve）を絶対にトリガーせず、処理を安全にスキップします。
    *   **【未解決スレッドの考慮（業界標準）】**: 正式な却下ボタンが押されていなくても、PR内に「未解決の会話スレッド（Unresolved Conversations）」が1件でも残っている場合は、開発者間で議論や修正が進行中（マージ保留中）とみなして自動承認を安全にスキップします。人間同士が納得し、会話がすべて「解決（Resolve）」にマークされたクリーンな状態に限り、自動承認の評価を再開します。人間の意思決定や対話はAIよりも常に優先されます。
    *   **【GraphQLによる物理スレッド解決状況の厳密チェック】**: GitHub REST APIではスレッドの解決（Resolved）ステータスを直接判定できないという仕様上の制限を回避するため、本アクションは内部で **GitHub GraphQL API** の `reviewThreads` クエリ（`isResolved` フィールド）を使用し、物理的なスレッドの解決状況を100%正確にチェックします。
...
graph TD
    ...
    ReviewScan -- "人間の CHANGES_REQUESTED または 未解決スレッドが1件でも存在" --> Skip["自動承認をスキップ / 監査チェックのみ通常終了"]
...
* **【ネガティブ／前提条件】** GitHub Actions上で自動Approveを実行するため、アクションが使用する `GITHUB_TOKEN` に対し、`pull-requests: write` の権限を設定（ワークフロー側に明示）する必要があります。
* **【ネガティブ／前提条件】** GitHubリポジトリ側の設定（**Settings ➔ Actions ➔ General ➔ Workflow permissions**）にて、**"Allow GitHub Actions to create and approve pull requests"** のチェックボックスをオンにする必要があります。この設定がオフの場合、APIは422 Unprocessable Entityエラーを返してしまい、自動承認処理が失敗します（本アクションではエラーをキャッチして安全にスキップ・通常終了します）。
